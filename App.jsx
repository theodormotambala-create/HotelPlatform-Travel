...
function CamBadge(props){
  ...
  return(<div onClick={function(e){e.stopPropagation();if(uploadRef&&uploadRef.current)uploadRef.current.click();}} style={{position:"absolute",bottom:0,right:0,width:sz,height:sz,borderRadius:"50%",background:DS.card,border:"2px solid "+DS.bg,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",boxShadow:"0 1px 4px rgba(0,0,0,.4)",zIndex:3}}><Camera size={Math.round(sz*.5)} color={DS.textMuted}/></div>);
}
  ...
  return(
    <div style={{position:"relative",width:outerSz,height:outerSz,flexShrink:0}}>
      ...
      <div style={{position:"absolute",top:ring,left:ring,width:sz,height:sz,borderRadius:"50%",border:"2.5px solid "+DS.bg,overflow:"hidden",boxSizing:"border-box"}}>
        <div onClick={onClickInner} style={{width:"100%",height:"100%",cursor:onClickInner?"pointer":"default"}}>
        ...
        {uploadRef&&<CamBadge uploadRef={uploadRef} sz={badgeSz}/>}
      </div>
      {verified&&<div style={{position:"absolute",bottom:ring-2,right:ring-2,background:DS.bg,borderRadius:"50%",padding:1,zIndex:4}}><VBadge sz={badgeSz}/></div>}
    </div>
  );
}
    ...
    <div style={{paddingBottom:20}}>
      ...
      <div style={{position:"relative",height:120,flexShrink:0}}>
        ...
        <div style={{position:"absolute",bottom:-44,left:14,zIndex:3}}>
          <DualAv sz={72} letter={(data.name[0]||"H").toUpperCase()} innerImg={profilePhoto} outerImg={data.img} verified={isVerified} uploadRef={_proUploadRef} onClickInner={profilePhoto?function(){_setProViewer(profilePhoto);}:null} onClickOuter={data.img?function(){_setProViewer(data.img);}:null}/>
        </div>
