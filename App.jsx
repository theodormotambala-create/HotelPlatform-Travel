...
function sanitizeText(str, maxLen){
  if(!str) return "";
  var clean = String(str).replace(/<\/?[a-zA-Z][^>]*>/g,"").replace(/javascript:/gi,"").trim();
  var limit = maxLen||1000;
  return clean.length > limit ? clean.slice(0, limit) : clean;
}
// Pile de retours universelle : chaque surface locale (conversation, feuille, visionneuse,
// menu...) s'enregistre a l'ouverture ; le retour physique/navigateur ferme TOUJOURS la
// surface du dessus, jamais l'application (pattern Facebook/Instagram).
var HPBack={_stack:[]};
function useBackClose(isOpen,closer){
  var ref=useRef(closer);ref.current=closer;
  useEffect(function(){
    if(!isOpen)return;
    var fn=function(){try{ref.current();}catch(e){}};
    HPBack._stack.push(fn);
    return function(){var i=HPBack._stack.lastIndexOf(fn);if(i>=0)HPBack._stack.splice(i,1);};
  },[isOpen]);
}
function fmtK(n){
...
var DataLayer = {
  ...
  syncFromSupabase: function(supabase){
    ...
    try{
      ...
      // 3 a 6. Chambres, plats, equipements et offres : plus AUCUN
      // telechargement global au demarrage.
      // Avant, ces quatre tables etaient lues EN ENTIER pour toute la
      // plateforme (select("*") sans borne), puis jointes aux etablissements
      // DANS le navigateur — une boucle sur chaque ligne avec une recherche
      // dans la liste des etablissements, donc un cout quadratique. A l'echelle
      // visee, c'est le catalogue mondial telecharge a chaque ouverture.
      // Desormais : le catalogue d'UN etablissement est charge a l'ouverture de
      // sa fiche (EstabM), borne par establishment_id et servi par les index
      // idx_rooms_estab / idx_dishes_estab. Les ecrans de configuration du pro
      // (HotelSvc, RestOff) chargeaient deja le leur de la meme facon.
      // Le prix « a partir de » de l'ecran Decouverte ne depend plus de ces
      // lignes : il est maintenu cote serveur sur establishments.price_from
      // (declencheurs price_from_sync), comme rating et review_count.
        ...
        if(res && res.data && res.data.length>0){
          try{
            ...
            if(newFromSb.length>0){
              ...
              localStorage.setItem(_lk("hp_resas_all"),JSON.stringify(merged));
              ...
              localStorage.setItem(_lk("hp_resas_all"),JSON.stringify(updated));
  ...
  updateReservationStatus: function(id, status, clientId){
    ...
    var VALID_STATUSES = ["confirmed","pending","refused","consumed","cancelled","completed"];
    ...
    <div style={{minHeight:"100vh",background:DS.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,animation:"hp-fade-up 0.28s ease"}}>
      ...
      <div style={{width:"100%",maxWidth:360}}>
        ...
        <div style={{position:"relative",marginBottom:10}}>
          ...
          <input type="text" inputMode="email" value={email} onChange={function(ev){setEmail(ev.target.value);}} onFocus={function(e){e.target.classList.add("hp-input-focus");}} onBlur={function(e){e.target.classList.remove("hp-input-focus");}} placeholder="Email" style={{width:"100%",background:DS.card,border:"1px solid "+DS.border,borderRadius:12,padding:"13px 16px 13px 38px",fontSize:13,color:DS.text,outline:"none",boxSizing:"border-box"}}/>
        </div>
        {mode!=="forgot"&&(
          <div style={{position:"relative",marginBottom:10}}>
...
function ChangePwdModal(props){
  ...
  async function submit(){
    ...
    if(pwd.length>72){setErr("Le mot de passe ne doit pas dépasser 72 caractères.");return;}
    if(pwd===curPwd){setErr("Le nouveau mot de passe doit être différent de l'actuel.");return;}
...
function SettingsS(props){
  ...
  var notifPrefs=props.notifPrefs||{reservation:true,message:true,comment:true,reaction:true,promo:true,follow:true};
...
function PremiumModal(props){
  var onClose=props.onClose;var onSubscribe=props.onSubscribe;var onSubscribeTrial=props.onSubscribeTrial;var accType=props.accType||"client";
...
function PrivacyModal(props){
  ...
  var LOCK_ROWS=isClientAcc?[
    ["Verrouiller mon profil","Nom et photo masqués dans le feed, commentaires et avis. Les établissements où vous réservez voient toujours votre vrai nom.",locked,function(v){onUpdate({locked:v});}],
...
function ChatUI(props){
  ...
  var onProfile=props.onProfile||null;
  var sPres=useState(null);var presence=sPres[0];var setPresence=sPres[1];
  ...
  var swipeRef=useRef({el:null,x0:0,y0:0,m:null,active:false});
  // Appui long (>=480ms) sur N'IMPORTE QUEL message non supprime -> menu d'actions (Facebook).
  function mlpStart(m,e){if(m.deleted)return;mlpTimer.current=setTimeout(function(){if(e&&e.cancelable)e.preventDefault();setMenuMsg(m);},480);}
  function mlpCancel(){if(mlpTimer.current){clearTimeout(mlpTimer.current);mlpTimer.current=null;}}
  // Glisser vers la DROITE sur un message -> repondre (identifier ce message), fluide.
  function swStart(m,e){if(m.deleted||!e.touches||!e.touches[0])return;var t=e.touches[0];swipeRef.current={el:e.currentTarget,x0:t.clientX,y0:t.clientY,m:m,active:false};}
  function swMove(e){var s=swipeRef.current;if(!s.el||!e.touches||!e.touches[0])return;var t=e.touches[0];var dx=t.clientX-s.x0;var dy=t.clientY-s.y0;if(!s.active&&Math.abs(dy)>Math.abs(dx)){return;}if(dx>0){s.active=true;mlpCancel();var d=Math.min(dx,72);s.el.style.transition="none";s.el.style.transform="translateX("+d+"px)";if(dx>8&&e.cancelable)e.preventDefault();}}
  function swEnd(e){var s=swipeRef.current;if(s.el){s.el.style.transition="transform .18s ease";s.el.style.transform="translateX(0px)";if(s.active){var t=e.changedTouches&&e.changedTouches[0];var dx=t?t.clientX-s.x0:0;if(dx>=56&&s.m&&!s.m.deleted)setReplyTo(s.m);}}swipeRef.current={el:null,x0:0,y0:0,m:null,active:false};}
  ...
  useBackClose(active!==null,function(){setActive(null);setReplyTo(null);});
  ...
  // Presence de l'interlocuteur : lue via get_presence (masquee si profil verrouille).
  useEffect(function(){
    setPresence(null);
    if(!otherUid||!DataLayer._client)return;
    var _f=function(){try{DataLayer._client.rpc("get_presence",{target:otherUid}).then(function(r){if(r&&!r.error)setPresence(r.data||null);}).catch(function(){});}catch(e){}};
    _f();var _iv=setInterval(_f,45000);
    return function(){clearInterval(_iv);};
  },[otherUid]);
  function _presInfo(){if(!presence)return null;var diff=Date.now()-new Date(presence).getTime();if(diff<2*60*1000)return{online:true,text:"en ligne"};return{online:false,text:"Vu "+timeAgo(presence)};}
  ...
  useEffect(function(){
    ...
    var chan=client.channel("conv:"+convId)
      ...
      .on("postgres_changes",{event:"UPDATE",schema:"public",table:"messages",filter:"conversation_id=eq."+convId},function(payload){
        // Temps reel : accuse de lecture ("Lu") et suppression par l'autre s'affichent en direct.
        var m=payload.new;
        setThr(function(ts){return ts.map(function(c){if(c.convId!==convId)return c;return Object.assign({},c,{msgs:c.msgs.map(function(x){return x.id===m.id?Object.assign({},x,{read:m.read,deleted:m.deleted,t:m.deleted?"[Message supprimé]":x.t}):x;})});});});
      }).subscribe();
    rtChan.current=chan;
  ...
  return(<div style={{display:"flex",flexDirection:"column",height:"100%",background:DS.bg}}><ToastC/><div style={{padding:"12px 16px",borderBottom:"1px solid "+DS.border,display:"flex",alignItems:"center",gap:10,background:DS.surface,flexShrink:0}}><BackBtn onClick={function(){setActive(null);setReplyTo(null);}}/><div style={{display:"flex",alignItems:"center",gap:10,flex:1,minWidth:0,cursor:(isClientChat&&onProfile&&otherUid)?"pointer":"default"}} onClick={function(){if(isClientChat&&onProfile&&otherUid)onProfile("prof_"+otherUid);}}><Av sz={38} letter={(conv[nK]||"?")[0]} img={iK?conv[iK]:null} verified={conv[vK]||false} isClient={isClientChat}/><div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:800,color:DS.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{conv[nK]}</div>{(function(){var _p=_presInfo();return(<div style={{fontSize:10,color:_p&&_p.online?DS.success:DS.textMuted,display:"flex",alignItems:"center",gap:4}}>{_p&&_p.online&&<span style={{width:7,height:7,borderRadius:"50%",background:DS.success,display:"inline-block",flexShrink:0}}/>}{_p?_p.text:"Membre HotelPlatform"}</div>);})()}</div></div>{otherUid&&<button onClick={toggleBlock} style={{padding:"6px 12px",borderRadius:16,border:"1px solid "+(iBlocked?DS.border:DS.error+"55"),background:iBlocked?DS.card:DS.errorSoft,color:iBlocked?DS.textMuted:DS.error,fontSize:11,fontWeight:700,cursor:"pointer",flexShrink:0}}>{iBlocked?"Débloquer":"Bloquer"}</button>}</div>{iBlocked&&<div style={{padding:"8px 16px",background:DS.warningSoft,borderBottom:"1px solid "+DS.warning+"33",fontSize:11,color:DS.warning,textAlign:"center",flexShrink:0}}>Vous avez bloqué ce compte — débloquez-le pour reprendre la conversation.</div>}<div style={{flex:1,minHeight:0,overflowY:"auto",WebkitOverflowScrolling:"touch",touchAction:"pan-y",padding:"12px 16px",display:"flex",flexDirection:"column",gap:6}}>{msgs.length===0&&<div style={{textAlign:"center",color:DS.textMuted,fontSize:12,marginTop:40}}>Debut de la conversation</div>}{msgs.map(function(m,i){var isMe=m.f==="me";return(<div key={m.id||i} style={{display:"flex",flexDirection:"column",alignItems:isMe?"flex-end":"flex-start",animation:"hp-msg-in 0.3s ease both",animationDelay:Math.min(i*20,200)+"ms"}}>{m.replyTo&&!m.deleted&&<div style={{padding:"4px 10px",background:DS.border,borderRadius:"8px 8px 0 0",fontSize:10,color:DS.textMuted,maxWidth:"75%",borderLeft:"3px solid "+myColor,marginBottom:-2}}><div style={{fontWeight:700,color:myColor}}>{m.replyTo.f==="me"?"Vous":conv[nK]}</div><div style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{m.replyTo.t}</div></div>}<div onDoubleClick={function(){if(!m.deleted)setReplyTo(m);}} onTouchStart={function(e){mlpStart(m,e);swStart(m,e);}} onTouchEnd={function(e){mlpCancel();swEnd(e);}} onTouchMove={function(e){mlpCancel();swMove(e);}} onTouchCancel={function(e){mlpCancel();swEnd(e);}} onMouseDown={function(){mlpStart(m);}} onMouseUp={mlpCancel} onMouseLeave={mlpCancel} onContextMenu={function(e){e.preventDefault();if(!m.deleted)setMenuMsg(m);}} style={{padding:"9px 13px",borderRadius:isMe?"18px 18px 4px 18px":"18px 18px 18px 4px",background:m.deleted?DS.border:isMe?myColor:DS.card,color:m.deleted?DS.textDim:isMe?"#fff":DS.text,fontSize:13,maxWidth:"75%",lineHeight:1.45,cursor:"pointer",fontStyle:m.deleted?"italic":"normal",whiteSpace:"pre-wrap",wordBreak:"break-word",userSelect:"none",WebkitUserSelect:"none",MozUserSelect:"none",msUserSelect:"none",WebkitTouchCallout:"none",willChange:"transform"}}>{m.deleted?"[Message supprimé]":m.t}</div><div style={{display:"flex",alignItems:"center",gap:4,marginTop:2}}><span style={{fontSize:9,color:DS.textDim}}>{m.time}</span>{isMe&&!m.deleted&&<span style={{fontSize:9,color:m.read?myColor:DS.textDim}}>{m.read?"Lu":"Envoyé"}</span>}</div></div>);})} </div>{replyTo&&<div style={{padding:"6px 16px",background:DS.surface,borderTop:"1px solid "+DS.border,display:"flex",alignItems:"center",gap:8,flexShrink:0}}><div style={{flex:1,borderLeft:"3px solid "+myColor,paddingLeft:8}}><div style={{fontSize:10,color:myColor,fontWeight:700}}>Répondre</div><div style={{fontSize:11,color:DS.textMuted,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{replyTo.t}</div></div><button onClick={function(){setReplyTo(null);}} style={{background:"none",border:"none",cursor:"pointer"}}><X size={14} color={DS.textMuted}/></button></div>}{qR&&msgs.length===0&&<div style={{padding:"6px 16px",display:"flex",gap:6,flexWrap:"wrap",flexShrink:0}}>{qR.map(function(q,i){return <button key={i} onClick={function(){setMsg(q);}} style={{padding:"5px 12px",borderRadius:20,border:"1px solid "+myColor+"44",background:myColor+"12",color:myColor,fontSize:11,cursor:"pointer"}}>{q}</button>;})} </div>}<div style={{padding:"10px 14px",borderTop:"1px solid "+DS.border,display:"flex",gap:8,alignItems:"center",background:DS.surface,flexShrink:0}}><textarea rows={1} value={msg} onChange={function(e){setMsg(e.target.value);e.target.style.height="auto";e.target.style.height=Math.min(e.target.scrollHeight,96)+"px";}} onFocus={function(e){e.target.classList.add("hp-input-focus");}} onBlur={function(e){e.target.classList.remove("hp-input-focus");}} placeholder={replyTo?"Répondre...":"Message..."} style={{flex:1,background:DS.card,border:"1px solid "+DS.border,borderRadius:20,padding:"10px 16px",fontSize:13,color:DS.text,outline:"none",resize:"none",maxHeight:96,overflowY:"auto",lineHeight:1.4,boxSizing:"border-box"}}/><button onClick={send} disabled={!msg.trim()||iBlocked} style={{width:40,height:40,borderRadius:"50%",background:(msg.trim()&&!iBlocked)?myColor:DS.border,border:"none",cursor:msg.trim()?"pointer":"default",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Send size={16} color="#fff"/></button></div>{menuMsg&&<ActionSheet label="ce message" onReply={function(){if(!menuMsg.deleted)setReplyTo(menuMsg);}} canDelete={menuMsg.f==="me"&&!menuMsg.deleted} onClose={function(){setMenuMsg(null);}} onDelete={function(){delMsg(menuMsg.id);}}/>}</div>);
}
...
function ShareSheet(props){
  ...
  var sCT=useState(null);var confirmTarget=sCT[0];var setConfirmTarget=sCT[1];
  ...
  // Logos officiels (SVG single-path, viewBox 24) — rendu identique aux grandes apps.
  var TARGETS=[
    ...
    {id:"facebook",name:"Facebook",color:"#1877F2",svg:"M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z",url:"https://www.facebook.com/sharer/sharer.php?u="+encodeURIComponent(shareUrl)+"&quote="+encodeURIComponent(shareText)},
    {id:"x",name:"X",color:"#000000",svg:"M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z",url:"https://twitter.com/intent/tweet?text="+encodeURIComponent(shareText)+"&url="+encodeURIComponent(shareUrl)},
    {id:"telegram",name:"Telegram",color:"#26A5E4",svg:"M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212-.07-.062-.174-.041-.249-.024-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z",url:"https://t.me/share/url?url="+encodeURIComponent(shareUrl)+"&text="+encodeURIComponent(shareText)},
    {id:"linkedin",name:"LinkedIn",color:"#0A66C2",svg:"M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.225 0z",url:"https://www.linkedin.com/sharing/share-offsite/?url="+encodeURIComponent(shareUrl)},
    {id:"email",name:"Email",color:"#5B6470",svg:"M22 6c0-1.1-.9-2-2-2H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6zm-2 0l-8 5-8-5h16zm0 12H4V8l8 5 8-5v10z",url:"mailto:?subject="+encodeURIComponent("HotelPlatform Travel")+"&body="+encodeURIComponent(full)}
  ];
  function _logo(t,sz){return(<svg viewBox="0 0 24 24" width={sz||26} height={sz||26} fill="#fff" aria-hidden="true"><path d={t.svg}/></svg>);}
  return(<div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",zIndex:1400,display:"flex",alignItems:"flex-end",justifyContent:"center",animation:"hp-fade 0.15s ease"}}>
    <div onClick={function(e){e.stopPropagation();}} style={{width:"100%",maxWidth:440,background:DS.surface,borderRadius:"20px 20px 0 0",border:"1px solid "+DS.border,animation:"hp-slide-up 0.24s ease",paddingBottom:8}}>
      ...
      {confirmTarget?(
        // DOUBLE CONFIRMATION : apercu du contenu partage + confirmation explicite (comme Facebook).
        <div style={{padding:"16px 18px 6px"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
            <div style={{width:44,height:44,borderRadius:"50%",background:confirmTarget.color,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{_logo(confirmTarget,24)}</div>
            <div style={{minWidth:0}}><div style={{fontSize:14,fontWeight:800,color:DS.text}}>Partager sur {confirmTarget.name}</div><div style={{fontSize:11,color:DS.textMuted,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{shareUrl}</div></div>
          </div>
          <div style={{background:DS.card,border:"1px solid "+DS.border,borderRadius:12,padding:"12px 14px",marginBottom:14}}>
            <div style={{fontSize:12,fontWeight:800,color:DS.text,marginBottom:3}}>{post.author}</div>
            <div style={{fontSize:12,color:DS.textMuted,lineHeight:1.5,display:"-webkit-box",WebkitLineClamp:3,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{post.text||"Publication"}</div>
          </div>
          <div style={{display:"flex",gap:8}}>
            <button onClick={function(){setConfirmTarget(null);}} style={{flex:1,padding:"12px",background:"transparent",border:"1px solid "+DS.border,borderRadius:12,cursor:"pointer",color:DS.textMuted,fontSize:13,fontWeight:700}}>Annuler</button>
            <button onClick={function(){var _u=confirmTarget.url;setConfirmTarget(null);openExt(_u);}} style={{flex:2,padding:"12px",background:confirmTarget.color,border:"none",borderRadius:12,cursor:"pointer",color:"#fff",fontSize:13,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>{_logo(confirmTarget,16)}Confirmer le partage</button>
          </div>
        </div>
      ):(
      <>
      <div style={{display:"flex",flexWrap:"wrap",padding:"16px 10px 6px"}}>
        {TARGETS.map(function(t){return(
          <button key={t.id} onClick={function(){setConfirmTarget(t);}} style={{width:"33.33%",background:"none",border:"none",cursor:"pointer",padding:"10px 4px",display:"flex",flexDirection:"column",alignItems:"center",gap:7}}>
            <div style={{width:50,height:50,borderRadius:"50%",background:t.color,display:"flex",alignItems:"center",justifyContent:"center"}}>{_logo(t,26)}</div>
    ...
    <div style={{width:"100%",maxWidth:420,height:"50vh",background:DS.surface,borderRadius:"20px 20px 0 0",border:"1px solid "+DS.border,display:"flex",flexDirection:"column",overflow:"hidden",animation:sheetAnim,transform:dragY>0?"translateY("+dragY+"px)":"none",transition:isDragging?"none":"transform 0.28s cubic-bezier(0.22,1,0.36,1)"}}>
      ...
      <div style={{display:"flex",gap:8,alignItems:"center",padding:"10px 14px",borderTop:"1px solid "+DS.border+"40",flexShrink:0,background:DS.surface}}>
        ...
        <div style={{flex:1,position:"relative"}}>
          <textarea rows={1} value={cmtText[post.id]||""} onChange={function(e){var nc=Object.assign({},cmtText);nc[post.id]=e.target.value;setCmtText(nc);e.target.style.height="auto";e.target.style.height=Math.min(e.target.scrollHeight,96)+"px";}} onFocus={function(e){e.target.classList.add("hp-input-focus");}} onBlur={function(e){e.target.classList.remove("hp-input-focus");}} placeholder={replyTo?"Répondre à "+replyTo.author+"...":"Ajouter un commentaire..."} style={{width:"100%",background:DS.card,border:"1px solid "+DS.border,borderRadius:20,padding:"10px 46px 10px 16px",fontSize:13,color:DS.text,outline:"none",boxSizing:"border-box",resize:"none",maxHeight:96,overflowY:"auto",lineHeight:1.4,display:"block"}}/>
...
function FeedText(props){
  ...
  return(
    <div style={{padding:"0 16px 12px",fontSize:15,color:DS.text,lineHeight:1.65,whiteSpace:"pre-wrap",wordBreak:"break-word"}}>
...
function ClientProf(props){
  ...
  useBackClose(_cpMenu,function(){_setCPMenu(false);_setCPPend(null);});
  useBackClose(_showEditClient,function(){_setShowEditClient(false);});
...
function EstabM(props){
  var e=props.e;var rawOnClose=props.onClose;var onBook=props.onBook;var onChat=props.onChat;var viewerIsPro=props.viewerIsPro||false;var selfUserId=props.selfUserId||null;var selfName=props.selfName||"Vous";
  try{
    var _h0=DataLayer.getHotels()[0];var _r0=DataLayer.getRestaurants()[0];
    if(_h0&&e&&e.id===_h0.id){
      var _rmsRaw=localStorage.getItem(_lk("hp_hotelsvc_rooms"));var _amsRaw=localStorage.getItem(_lk("hp_hotelsvc_amenities"));var _dshRaw=localStorage.getItem(_lk("hp_hotelsvc_dishes"));var _descRawH=localStorage.getItem(_lk("hp_pro_desc_hotel"));
      var _patch={};
      ...
      var _itRaw=localStorage.getItem(_lk("hp_restoff_items"));var _ofRaw=localStorage.getItem(_lk("hp_restoff_offers"));var _descRawR=localStorage.getItem(_lk("hp_pro_desc_restaurant"));
      var _patch2={};
  ...
  // Catalogue de CET etablissement (chambres, plats, equipements, offres),
  // charge a l'ouverture de la fiche — meme patron que les avis ci-dessus.
  // Il remplace le telechargement GLOBAL du catalogue de toute la plateforme
  // au demarrage (DataLayer.syncFromSupabase, etapes 3 a 6), qui chargeait
  // establishment_rooms / _dishes / _amenities / _offers en entier, sans
  // aucune borne, et joignait ensuite les lignes aux etablissements DANS le
  // navigateur. Ici la lecture est bornee a un seul etablissement et s'appuie
  // sur les index existants idx_rooms_estab / idx_dishes_estab.
  // Tant que la reponse n'est pas arrivee, les valeurs deja presentes sur « e »
  // restent affichees : aucun ecran ne se vide.
  var sCat=useState(null);var catalogue=sCat[0];var setCatalogue=sCat[1];
  useEffect(function(){
    if(!DataLayer._client||!e||!e.id)return;
    var annule=false;
    Promise.all([
      DataLayer._client.from("establishment_rooms").select("id,name,price,capacity,available,stock,description").eq("establishment_id",e.id),
      DataLayer._client.from("establishment_dishes").select("id,name,price,category,description,available").eq("establishment_id",e.id),
      DataLayer._client.from("establishment_amenities").select("id,name,active").eq("establishment_id",e.id),
      DataLayer._client.from("establishment_offers").select("id,name,price,available").eq("establishment_id",e.id)
    ]).then(function(rs){
      if(annule)return;
      var _r=(rs[0]&&rs[0].data)||null, _d=(rs[1]&&rs[1].data)||null;
      var _a=(rs[2]&&rs[2].data)||null, _o=(rs[3]&&rs[3].data)||null;
      var _m=null;
      if(_d){ // regroupement par categorie, identique a celui du chargement global
        var _c={};_d.forEach(function(x){var k=x.category||"Plats";if(!_c[k])_c[k]=[];_c[k].push(x);});
        _m=Object.keys(_c).map(function(k){return{cat:k,items:_c[k]};});
      }
      setCatalogue({
        rooms:_r, menu:_m,
        services:_a?_a.filter(function(x){return x.active!==false;}):null,
        offers:_o?_o.filter(function(x){return x.available!==false;}):null
      });
    }).catch(function(){});
    return function(){annule=true;};
  },[e&&e.id]);
  var _catRooms   =(catalogue&&catalogue.rooms)   ||e.rooms   ||[];
  var _catMenu    =(catalogue&&catalogue.menu)    ||e.menu    ||[];
  var _catServices=(catalogue&&catalogue.services)||e.services||[];
  var _catOffers  =(catalogue&&catalogue.offers)  ||e.offers  ||[];
  var REVIEWS_PAGE=20;
  ...
  return(<div style={{position:"fixed",inset:0,background:DS.bg,zIndex:900,maxWidth:420,margin:"0 auto",overflowY:"auto",WebkitOverflowScrolling:"touch",touchAction:"pan-y",animation:(closingE?"hp-slide-out-right 0.26s cubic-bezier(0.4,0,1,1) forwards":"hp-slide-right 0.32s cubic-bezier(0.22,1,0.36,1)"),boxShadow:"-8px 0 24px rgba(0,0,0,.35)"}}><ImgViewer src={_eViewer} onClose={function(){_setEViewer(null);}}/><Toast/><div style={{position:"relative",height:220,flexShrink:0}}><img src={e.img} alt="" onClick={function(){if(e.img)_setEViewer(e.img);}} style={{width:"100%",height:"100%",objectFit:"cover",cursor:"pointer"}}/><div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,rgba(0,0,0,.2),rgba(0,0,0,.6))",pointerEvents:"none"}}/><div style={{position:"absolute",top:12,left:12}}><BackBtn onClick={onClose} light={true}/></div><div style={{position:"absolute",bottom:-48,left:16,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2}} onClick={function(){if(e.img)_setEViewer(e.img);}}><Av sz={68} letter={(e.name[0]||"H").toUpperCase()} img={e.img} verified={e.verified||false}/>{e.verified&&<div style={{background:"#14532d",borderRadius:6,padding:"1px 6px",marginTop:1}}><span style={{fontSize:9,fontWeight:800,color:"#4ade80",letterSpacing:"0.01em"}}>Vérifié</span></div>}</div></div><div style={{padding:"58px 16px 8px"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}><div><div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><div style={{fontSize:20,fontWeight:900,color:DS.text}}>{e.name}</div></div><a href={"https://maps.google.com/?q="+encodeURIComponent(e.name+" "+e.location)} target="_blank" rel="noopener noreferrer" style={{fontSize:12,color:DS.primary,cursor:"pointer",display:"flex",alignItems:"center",gap:4,textDecoration:"none"}}><MapPin size={11}/>{e.location}</a></div><div style={{textAlign:"right"}}>{e.priceFrom?<div style={{fontSize:18,fontWeight:900,color:DS.gold}}>À partir de {e.priceFrom}€</div>:null}</div></div><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}><Stars r={e.rating} sz={15}/><span style={{fontSize:13,fontWeight:800,color:DS.text}}>{e.rating}</span><span style={{fontSize:13,color:DS.textDim}}>-</span><span style={{fontSize:13,fontWeight:700,color:DS.text}}>{fmtK(followersCount)}</span><span style={{fontSize:12,color:DS.textMuted}}>abonnes</span></div><div style={{display:"flex",gap:8,marginBottom:16}}>{viewerIsPro
            ...
            <div>
              ...
              {(_catRooms||[]).filter(function(r){return r.available;}).map(function(r){
            ...
            <div>
              ...
              {(_catMenu||[]).map(function(cat,ci){return(
...
function StripePaymentModal(props){
  ...
  useBackClose(!!(_stripePromise&&clientSecret),onClose);
      ...
      {posts.slice(0,visibleCountPro).map(function(post,_pfi){
        ...
        return(
          ...
          <div ref={function(el){postRefsPro.current[post.id]=el;}} style={{background:DS.surface,marginBottom:10,borderTop:"1px solid "+DS.border+"28",borderBottom:"1px solid "+DS.border+"28",animation:"hp-item-in 0.34s ease both",animationDelay:(_pfi*50)+"ms"}}>
            ...
            <div style={{padding:"0 16px 16px",fontSize:15,color:DS.text,lineHeight:1.7,whiteSpace:"pre-wrap",wordBreak:"break-word"}}>{post.text}</div>
...
function HotelSvc(props){
  ...
  var color=DS.hotel;
  var s1=useState(function(){try{var v=localStorage.getItem(_lk("hp_hotelsvc_rooms"));return v?JSON.parse(v):(data.rooms||[]);}catch(e){return data.rooms||[];}});var rooms=s1[0];var setRooms=s1[1];
  var s2=useState(null);var editItem=s2[0];var setEditItem=s2[1];
  var s3=useState(false);var showAdd=s3[0];var setShowAdd=s3[1];
  var s4=useState(function(){try{var v=localStorage.getItem(_lk("hp_hotelsvc_svcmode"));return v||data.svcMode||"hotel";}catch(e){return data.svcMode||"hotel";}});var svcMode=s4[0];var setSvcMode=s4[1];
  var hasResto=svcMode==="combined";
  var s5=useState(function(){try{var v=localStorage.getItem(_lk("hp_hotelsvc_dishes"));return v?JSON.parse(v):[];}catch(e){return[];}});var menu=s5[0];var setMenu=s5[1];
  var s6=useState(data.svcMode==="restaurant"?"menu":"rooms");var tab=s6[0];var setTab=s6[1];
  var sa=useState(function(){try{var v=localStorage.getItem(_lk("hp_hotelsvc_amenities"));return v?JSON.parse(v):(data.services||[]).map(function(s,i){return typeof s==="object"?Object.assign({},s):{id:"svc"+i,name:s,active:true};});}catch(e){return(data.services||[]).map(function(s,i){return typeof s==="object"?Object.assign({},s):{id:"svc"+i,name:s,active:true};});}});
  ...
  useEffect(function(){
    ...
    DataLayer._client.from("establishment_rooms").select("*").eq("establishment_id",_hEstabId)
      .then(function(res){
        ...
        setRooms(rs);try{localStorage.setItem(_lk("hp_hotelsvc_rooms"),JSON.stringify(rs));}catch(e){}
      }).catch(function(){});
    DataLayer._client.from("establishment_dishes").select("*").eq("establishment_id",_hEstabId)
      .then(function(res){
        ...
        setMenu(ds);try{localStorage.setItem(_lk("hp_hotelsvc_dishes"),JSON.stringify(ds));}catch(e){}
      }).catch(function(){});
    DataLayer._client.from("establishment_amenities").select("*").eq("establishment_id",_hEstabId)
      .then(function(res){
        ...
        setAmenities(am);try{localStorage.setItem(_lk("hp_hotelsvc_amenities"),JSON.stringify(am));}catch(e){}
      }).catch(function(){});
  },[_hEstabId]);
...
function RestOff(props){
  ...
  var s1=useState(function(){try{var v=localStorage.getItem(_lk("hp_restoff_items"));return v?JSON.parse(v):_defaultItems;}catch(e){return _defaultItems;}});
  ...
  var s4=useState(function(){try{var v=localStorage.getItem(_lk("hp_restoff_offers"));return v?JSON.parse(v):_defaultOffers;}catch(e){return _defaultOffers;}});
  ...
  useEffect(function(){
    ...
    DataLayer._client.from("establishment_dishes").select("*").eq("establishment_id",_rEstabId)
      .then(function(res){
        if(res.error||!res.data||res.data.length===0)return;
        var ds=res.data.map(function(d){return{id:d.id,name:d.name,price:d.price||0,category:d.category||"Plats",description:d.description||null,available:d.available!==false};});
        setItems(ds);try{localStorage.setItem(_lk("hp_restoff_items"),JSON.stringify(ds));}catch(e){}
      }).catch(function(){});
    ...
    <div style={{paddingBottom:20}}>
      ...
      <div style={{padding:"56px 16px 10px"}}>
        ...
        {tab==="services"&&(function(){
          var _svcs=[];
          try{
            if(proType==="hotel"){var va=localStorage.getItem(_lk("hp_hotelsvc_amenities"));_svcs=va?JSON.parse(va).filter(function(a){return a.active;}):(data.services||[]);}
            else{var vb=localStorage.getItem(_lk("hp_restoff_offers"));_svcs=vb?JSON.parse(vb):(data.offers||[]);}
...
export default function App() {
  ...
  useEffect(function(){
    ...
    var sub = sb.auth.onAuthStateChange(function(event, session){
      if(event==="SIGNED_IN" && session && session.user){
        if(!DataLayer._client&&typeof window!=="undefined"&&window.__supabase){DataLayer._client=window.__supabase;}
        var meta = session.user.user_metadata || {};
  ...
  // « Essayez Premium » : essai 15 jours — circuit SEPARE du mensuel (webhook -> grant_premium_days).
  function subscribeTrial(plan){
    var _pr=premiumPricing&&premiumPricing.prices;
    var trialP=(_pr&&_pr.trial15!=null&&!isNaN(Number(_pr.trial15)))?Number(_pr.trial15):4.99;
    var cents=Math.round(trialP*100);
    if(!_stripePromise){tk.show("Paiement indisponible : Stripe non configuré","error");return;}
    if(!cents||cents<50){tk.show("Montant invalide","error");return;}
    setShowPremium(false);
    fetch("/api/create-payment-intent",{
  ...
  // Presence "en ligne" : heartbeat serveur (touch_presence) toutes les 60s tant que
  // le compte est actif (onglet visible). Additif — n'affecte aucune autre logique.
  useEffect(function(){
    var uid=_authForUserData&&_authForUserData.userId;
    if(!uid||!DataLayer._client)return;
    var _beat=function(){try{if(DataLayer._client)DataLayer._client.rpc("touch_presence").then(function(){}).catch(function(){});}catch(e){}};
    _beat();
    var _iv=setInterval(function(){if(typeof document==="undefined"||!document.hidden)_beat();},60000);
    return function(){clearInterval(_iv);};
  },[_authForUserData&&_authForUserData.userId]);
  ...
  useEffect(function(){
    ...
    function onPop(){
      if(HPBack._stack.length>0||overlayRef.current){
  ...
  // (place AVANT tout return conditionnel : regle des hooks React)
  useEffect(function(){
    ...
    try{
      ...
      if(pid){
        ...
        if(auth.type==="client")setCTab("feed");else setPTab("feed");
  ...
  var _accountOverlays=(<>
    {showChangeEmail&&<ChangeEmailModal accent={accent} onClose={function(){setShowChangeEmail(false);}}/>}
    {showChangePwd&&<ChangePwdModal accent={accent} onClose={function(){setShowChangePwd(false);}} onSuccess={function(){toastApp("Mot de passe mis à jour avec succes","success");}}/>}
  </>);
  if(sett)       return <>
