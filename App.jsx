...
function CamOverlay(props){
  var sz=props.sz;var uploadRef=props.uploadRef;
  var sh=useState(false);var hov=sh[0];var setHov=sh[1];
  return(<div onClick={function(e){e.stopPropagation();if(uploadRef.current)uploadRef.current.click();}} onMouseEnter={function(){setHov(true);}} onMouseLeave={function(){setHov(false);}} onTouchStart={function(){setHov(true);}} onTouchEnd={function(){setHov(false);}} style={{position:"absolute",inset:0,background:hov?"rgba(0,0,0,.42)":"rgba(0,0,0,0)",display:"flex",alignItems:"center",justifyContent:"center",borderRadius:"50%",transition:"background .18s",cursor:"pointer"}}><Camera size={Math.round(sz*.28)} color="#fff" style={{opacity:hov?1:0,transition:"opacity .18s"}}/></div>);
}
function DualAv(props){
  var sz=props.sz||72;var letter=props.letter||"?";var innerImg=props.innerImg||null;var outerImg=props.outerImg||null;var verified=props.verified||false;var onClickInner=props.onClickInner||null;var onClickOuter=props.onClickOuter||null;var uploadRef=props.uploadRef||null;
  var ring=8;var outerSz=sz+ring*2;
  return(
    <div style={{position:"relative",width:outerSz,height:outerSz,flexShrink:0}}>
      {outerImg&&<div onClick={onClickOuter} style={{position:"absolute",inset:0,borderRadius:"50%",overflow:"hidden",cursor:onClickOuter?"pointer":"default",flexShrink:0}}>
        <img src={outerImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover",borderRadius:"50%",opacity:.72}}/>
        <div style={{position:"absolute",inset:0,borderRadius:"50%",boxShadow:"inset 0 0 0 2px rgba(255,255,255,.18)"}}/>
      </div>}
      {!outerImg&&<div style={{position:"absolute",inset:0,borderRadius:"50%",background:DS.card,border:"2px solid "+DS.border}}/>}
      <div onClick={onClickInner} style={{position:"absolute",top:ring,left:ring,width:sz,height:sz,borderRadius:"50%",overflow:"hidden",cursor:onClickInner?"pointer":"default",border:"2px solid "+DS.bg,boxSizing:"border-box"}}>
        {innerImg?<img src={innerImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<div style={{width:"100%",height:"100%",background:DS.primary+"30",display:"flex",alignItems:"center",justifyContent:"center",fontSize:Math.round(sz*.4),fontWeight:800,color:DS.primary}}>{letter}</div>}
        {uploadRef&&<CamOverlay sz={sz} uploadRef={uploadRef}/>}
      </div>
      {verified&&<div style={{position:"absolute",bottom:ring-4,right:ring-4,background:DS.bg,borderRadius:"50%",padding:1,zIndex:2}}><VBadge sz={Math.round(sz*.34)}/></div>}
    </div>
  );
}
            ...
            <div style={{display:"flex",alignItems:"flex-start",gap:12,padding:"14px 16px 10px"}}>
              <div style={{display:"flex",alignItems:"flex-start",gap:12,flex:1,minWidth:0}}>
                <div onClick={function(){if(onProfile)onProfile(post.id,post.type);}} style={{cursor:"pointer",flexShrink:0}}>
                  <Av sz={52} letter={post.author[0]} img={function(){var _e=DataLayer.getHotels().concat(DataLayer.getRestaurants()).find(function(x){return x.id===post.id;});return _e?_e.img:null;}()} verified={post.verified}/>
                </div>
...
function BookM(props){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",zIndex:1100,display:"flex",alignItems:"flex-end",justifyContent:"center",animation:"hp-fade 0.2s ease"}}>
      <div style={{width:"100%",maxWidth:420,background:DS.surface,borderRadius:"22px 22px 0 0",border:"1px solid "+DS.border,maxHeight:"94vh",overflowY:"auto",animation:closing?"hp-sheet-out 0.26s ease forwards":"hp-slide-up 0.3s ease"}}>
        <Toast/>
...
function ProProf(props){
  ...
  return(
    <div style={{paddingBottom:20}}>
      ...
      <div style={{padding:"0 16px 0",marginTop:-44,display:"flex",alignItems:"flex-end",gap:12,position:"relative",zIndex:2}}>
        <DualAv sz={72} letter={(data.name[0]||"H").toUpperCase()} innerImg={profilePhoto} outerImg={data.img} verified={isVerified} uploadRef={_proUploadRef}/>
      </div>
